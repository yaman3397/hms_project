...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_discharge'])) {
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>window.location.href='discharge.php';</script>";
        exit;
    } else {
  ...
  <div class="glass-card">
    ...
    <form method="POST" class="row g-3">
      ...
      foreach ($fields as [$key, $label]) {
          $val = $data[$key] ?? '';
          $type = 'text';
          if (in_array($key, ['admission_date', 'discharge_date'])) $type = 'date';
          elseif (in_array($key, ['admission_time', 'discharge_time'])) $type = 'time';

          $readonly = in_array($key, ['admission_date', 'admission_time']) ? 'readonly' : '';
          echo "<div class='col-md-4'><label>$label</label><input type='$type' class='form-control' name='$key' value='".htmlspecialchars($val ?? '')."' $readonly></div>";
      }
      ?>
