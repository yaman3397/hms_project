...
if (!empty($conditions)) {
    ...
    $sort_order = ($_GET['sort'] ?? 'desc') === 'asc' ? 'ASC' : 'DESC';
    $result = $conn->query("SELECT * FROM patients ORDER BY created_at $sort_order LIMIT $limit OFFSET $offset");
}
...
<html>
<head>
    <title>Children Hospital - OPD Patient List</title>
    ...
    for (let i = 2; i <= 16; i++) {
        ...
        if (i === 4) {
            field.addEventListener('change', () => {
                ...
                if (!isNaN(dob)) {
                    const ageCell = row.children[5];
                    const age = calculateAge(dob);
                    const ageInput = document.createElement('input');
                    ageInput.type = 'text';
                    ageInput.className = 'form-control form-control-sm';
                    ageInput.value = age;
                    ageCell.innerHTML = '';
...
function saveRow(row, mrNumber, button) {
    const fields = [
        'name', 'father_name', 'dob', 'age', 'sex', 'mobile', 'alt_mobile',
    ...
    for (let i = 0; i < fields.length; i++) {
        const input = row.children[i + 2].querySelector('input');
        data.fields[fields[i]] = input.value;
    }
