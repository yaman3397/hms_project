...
$query = "SELECT ir.*, p.name, p.dob, p.sex, p.age, p.address, p.department, p.mobile, p.category FROM gynae_ipd_records ir LEFT JOIN gynae_patients p ON ir.mr_number = p.mr_number";
$conditions = [];
$params = [];
$types = '';

if (!empty($mr_number)) {
    $conditions[] = "ir.mr_number LIKE ?";
    $params[] = "%" . $mr_number . "%";
    $types .= 's';
}
if (!empty($ipd_no)) {
    $conditions[] = "ir.ipd_no LIKE ?";
    $params[] = "%" . $ipd_no . "%";
    $types .= 's';
}
if (!empty($doctor)) {
    $conditions[] = "ir.doctor_name LIKE ?";
    $params[] = "%" . $doctor . "%";
    $types .= 's';
}
...
$count_query = "SELECT COUNT(*) as total FROM gynae_ipd_records ir LEFT JOIN gynae_patients p ON ir.mr_number = p.mr_number ";
if (!empty($conditions)) {
    ...
    $total_result = $conn->query("SELECT COUNT(*) as total FROM gynae_ipd_records ir LEFT JOIN gynae_patients p ON ir.mr_number = p.mr_number");
    $total_records = $total_result->fetch_assoc()['total'];
    $sort_order = ($_GET['sort'] ?? 'desc') === 'asc' ? 'ASC' : 'DESC';
    $result = $conn->query("SELECT ir.*, p.name, p.dob, p.sex, p.age, p.address, p.department, p.mobile, p.category FROM gynae_ipd_records ir LEFT JOIN gynae_patients p ON ir.mr_number = p.mr_number ORDER BY ir.id $sort_order LIMIT $limit OFFSET $offset");
}
