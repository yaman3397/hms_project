...
function generateIPDNumber($conn) {
    ...
    $result = $conn->query("SELECT MAX(id) as max_id FROM gynae_ipd_records");
    $max_id = $result ? (int) $result->fetch_assoc()['max_id'] : 0;
    $next = str_pad($max_id + 1, 4, '0', STR_PAD_LEFT);
  ...
  <div class="glass-card">
    ...
    <form class="row g-3" method="POST">
      ...
      <div class="col-md-4">
        ...
        <input type="text" name="mr_number" class="form-control" value="<?= htmlspecialchars($patient['mr_number'] ?? '') ?>" required readonly>
      </div>
      <div class="col-md-4">
        <label>IPD No <span class="text-danger">*</span></label>
        <input type="text" name="ipd_no" class="form-control" value="<?= $ipd_no ?>" required readonly>
      </div>
      <div class="col-md-4">
